...
(function () {
    ...
    function _icon(name, size, extraClass) {
        ...
        if (!path) {
            var _sh = window.CT_ICONS;
            if (_sh)
                path = _sh[name];
        } // fall back to shared CT_ICONS (moon, sun, settings…)
        if (!path)
            return "";
        var sz = size || 16;
    ...
    function _dashStats() {
        ...
        var byStatus = { new: 0, false_positive: 0, to_fix: 0, fixed: 0, closed_upstream: 0, derogated: 0 };
        var active = [];
    ...
    function _dashTimeline() {
        ...
        function yFor(v) {
            if (useLog)
                return MT + cH - (Math.log((v || 0) + 1) / logMax) * cH;
    ...
    window._hostsSetView = function (view) {
        _hostsView = view === "table" ? "table" : "cards";
        try {
            localStorage.setItem("surface.hostsView", _hostsView);
        }
        catch (e) { }
        var c = document.getElementById("content");
        if (c)
    ...
    function _hostsTableHtml(entries, counts) {
        ...
        entries.forEach(function (entry) {
            ...
            if (a.label)
                h += '<div class="ct-muted ct-text-meta">' + esc(a.label) + '</div>';
            h += '</td>';
