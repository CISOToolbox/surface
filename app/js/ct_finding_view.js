...
(function() {
    ...
    function _renderInfoCard(f, opts) {
        ...
        if (f.scanner)  row(_t("fd.scanner", "Scanner"), f.scanner);
        if (f.type)     row(_t("fd.type", "Type"),    f.type);
        row(_t("fd.target", "Cible"), f.target || "-", { style: "font-family:monospace;word-break:break-all" });
        if (f.cve_id)   row("CVE", f.cve_id);
        if (f.application_name) row(_t("findings.application", "Application"), f.application_name);
        if (f.created_at) row(_t("fd.created", "Créé le"), _fmtDate(f.created_at));
        if (f.triaged_at) {
    ...
    function _renderInfoCardClean(f, opts) {
        ...
        if (ev && typeof ev === "object" && Object.keys(ev).length) {
            ...
            if (opts.showScreenshot !== false && ev.png_b64 && typeof ev.png_b64 === "string") {
                var src = "data:image/png;base64," + ev.png_b64;
                row(
                    _t("fd.screenshot", "Capture d'écran"),
            ...
            row(
                _t("fd.evidence", "Preuves"),
                '<pre style="background:#f9fafb;padding:8px;border-radius:4px;font-size:0.75em;overflow:auto;max-height:240px">'
                + esc(JSON.stringify(evDisplay, null, 2))
                + '</pre>',
                true
            );
        ...
        if (Array.isArray(opts.infoRows)) {
            opts.infoRows.forEach(function(r) {
                if (!r || r.label == null) return;
                if (r.valueHtml != null) row(r.label, r.valueHtml, true, r.style);
                else row(r.label, r.value, false, r.style);
            });
        }
    ...
    function _renderLinkedMeasureCard(m, opts) {
        ...
        var meta = [];
        if (statusLbl)    meta.push(esc(_t("fd.measure_status", "Statut"))    + ' : ' + esc(statusLbl));
        if (m.responsable) meta.push(esc(_t("fd.measure_owner", "Responsable")) + ' : ' + esc(m.responsable));
        if (m.echeance)    meta.push(esc(_t("fd.measure_due", "Échéance"))    + ' : ' + esc(m.echeance));
        if (meta.length) {
    ...
    function render(finding, opts) {
        ...
        if (opts.subheaderHtml) h += opts.subheaderHtml;
