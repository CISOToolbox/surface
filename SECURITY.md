...
- **Sessions are issued and verified by this module alone.** `JWT_SECRET` is a
  ...
  `HKDF-SHA256(JWT_SECRET, salt="ciso-suite/jwt-key/v1", info="ciso-module:surface")`
  and tokens carry `aud=ciso-module:surface`. A token minted by another module -
...
- **`ENCRYPTION_KEY` is mandatory and dedicated.** Credentials stored at
  rest are encrypted with it, and there is no fallback on `JWT_SECRET`, so
  the session and data-at-rest trust domains stay separate. Rotating it
  requires re-entering the stored credentials.
...
1. Generate every secret separately with `openssl rand -hex 32`. Never reuse a
   value across `JWT_SECRET`, `AUTH_TOKEN`, `ENCRYPTION_KEY` and `DB_PASSWORD`.
