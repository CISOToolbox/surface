# End-to-end tests - Surface (standalone)

These tests drive a **real running Surface stack** over HTTP. Nothing is mocked.
...
```bash
docker compose up -d
E2E_BASE_URL=http://localhost:8087 python3 -m pytest -v tests/e2e
```
...
|----------|---------|---------|
| `E2E_BASE_URL` | `http://localhost:8087` | where the module answers |
...
(`/surface/`). A standalone deployment has none of that - it serves the module at
`/`, mints a `surface_token` cookie signed with a key derived per module, and logs
