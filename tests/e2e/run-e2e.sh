#!/usr/bin/env bash
# -----------------------------------------------------------------------------
#  run-e2e.sh - end-to-end tests for the Surface standalone deployment
# -----------------------------------------------------------------------------
#
...
#
#  Environment:
#      E2E_BASE_URL     default http://localhost:8087
...
BASE_URL="${E2E_BASE_URL:-http://localhost:8087}"
KEEP=0
DO_UP=1
for arg in "$@"; do
    case "$arg" in
...
if [ "$DO_UP" = "1" ]; then
    ...
    echo "== starting the Surface stack"
