"""Shared fixtures for the Surface standalone end-to-end tests.
...
Configuration (all optional; the defaults match `docker compose up`):
    ...
    E2E_BASE_URL     base URL of the running module  (default http://localhost:8087)
...
def client(base_url):
    ...
    if not _wait_for_health(c, float(os.getenv("E2E_BOOT_TIMEOUT", "60"))):
        pytest.skip(
            "no surface instance answering on " + base_url +
